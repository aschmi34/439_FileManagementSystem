...
printf("\ncurrent dir: %s", copy->filename);
    while (prev_node != NULL)
    {
        ...
        if (strcmp(("%s", prev_node->left->filename), ("%s", copy->filename)))
        {
            printf("\ncurrent path: %s/%s", prev_node->filename, new_data);
            if (prev_node->left->left != NULL && prev_node->left->right != NULL)
            {
            }
            else
            {
                
                prev_node->left->left = deleteLNode(prev_node->left->left, new_data);
    ...
    while (prev_node->right != NULL)
    {
        ...
        if (strcmp(("%s", prev_node->right->filename), ("%s", copy->filename)))
        {
            ...
            else
            {
                ...
                prev_node->right->right = deleteLNode(prev_node->right->right, new_data);
                // prev_node->right->right = makeTdir(1, prev_node->right, new_data);
