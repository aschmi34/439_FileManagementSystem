...
list. */
void pushdir(struct fileNode** head_ref, char new_data[])
{
	makeTdir(new_data);

}

struct fileNode* createNode(char new_data[]) {
...
void insertRight(struct fileNode* prev_node, char new_data[])
{
...
//     new_node->left= NULL;
	...
	prev_node->right = createNode(new_data);
...
void insertleft(struct fileNode* prev_node, char new_data[])
{
		/*1. check if the given prev_node is NULL */
	if (prev_node == NULL) {
		printf("the given previous node cannot be NULL");
