...
struct fileNode* makeTdir(int root,char new_data[])
{
         ...
         for (size_t i = 0; i < strlen(branches); i++)
    {
     ...
     for (size_t i = ; i < strlen(new_data); i++)
    {
...
if(root==0){
    ...
    printf("3rd: %s\n",new_node->filename);
    if (stat(("/home/jtorne2/%s",new_node->filename), &st) == -1) {
    mkdir(("/home/jtorne2/%s",new_node->filename), 0777);
}
}else{
    
    printf("4th: %s\n",new_node->filename);
if (stat(("/home/jtorne2/rootDir/%s",new_node->filename), &st) == -1) {
    mkdir(("/home/jtorne2/rootDir/%s",new_node->filename), 0777);
}
}
