...
list. */
void pushdir(struct dirNode** head_ref, char new_data[])
{

	/* 1. allocate node */
	struct dirNode* new_node
		= (struct 
...
if (stat("/home/jtorne2/.idea/", &st) == -1) {
...
void push(struct dirNode** head_ref, char new_data[])
{
...
* given node */
void insertAfter(struct dirNode* prev_node, char new_data[])
{
...
void append(struct dirNode** head_ref, char new_data[])
{
	...
	struct dirNode* last = *head_ref; /* used in step 5*/
...
// from the given node
void printList(struct dirNode* node)
{

	struct dirNode* last;
	printf("\nTrav
    ersal in forward direction \n");
	while (node != NULL) {
