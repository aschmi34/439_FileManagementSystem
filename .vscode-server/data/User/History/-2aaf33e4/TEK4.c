...
char *changeDir(int direction, struct fileNode *curDir, char node[100], char newDir[100])
{
  ...
  if (strncmp(("%s", curDir->filename), ("/home/jtorne2/rootDir"), 21) == 0)
  {
    ...
    if(strcmp(newDir,"/home/jtorne2/rootDir")){
      return node;
    }
    strcat(node, "/");
    strcat(node, newDir);
...
void main()
{
  ...
  int ans = -2;
  printf("Hello.\n\n");
  struct fileNode *root = pushdir(0,root, "rootDir");
    ...
    { // Option 6 rename file or directory xxxxxxxxxxxxxxx
      ...
      else if (ans == 2)
      {
        if (strncmp(("%s", copy->filename), ("/home/jtorne2/rootDir"), 21) == 0)
        {
          ...
          printf("\nWhat would you like to rename the current directory '%s' to? ", root->filename);
          scanf("%s", str);
          rename(copy->filename, str);
          printf("\ncurrent directory changed to '%s': ", copy->filename);
          memset(dir,0,100);
             for (size_t i = 0; i < strlen(str); i++)
            {
              dir[i] = str[i];
