...
char *traverseDir(int direction, struct fileNode *curDir, struct fileNode *prevDir, char node[])
{
  ...
  else if (strncmp(("%s", curDir->filename), (head), 21) == 0 && direction == 2)
  {
...
void deleteAll(struct fileNode *node, struct fileNode *copy)
{
  ...
  while (last != NULL)
  {
    ...
    if (("%s", last->right) == NULL)
    {
      ...
      removeleftD(root, ("%s", last->filename),copy);
      }else{
      removeleft(root, ("%s", last->filename),copy);
    ...
    else
    {
      ...
      removeRightD(root, ("%s", last->filename),copy);
      }else{
      removeRight(root, ("%s", last->filename),copy);
