...
void push(struct dirNode** prev_node, char new_data[])
{
...
char dir[]=("%s",prev_node -> filename);
char str[]=("%s",new_node -> filename);
...
//  char number[]=();
 FILE* input = fopen(("%s",new_node->filename),"w");
 fprintf(input,"%s", new_node->filename);
